    ...
    $.fn.graphview = function(options) {
        ...
        var setnodesandlinks = function() {
            ...
            $('.graphview_nodetype:checked', obj).each(function() {
                ...
                var obj = options.response.aggregations[key];
        ...
        var ranged = function() {
            ...
            if ( $('.dateranged', obj).length == 0 ) { 
                ...
                var ranged_values = [];
                var entries = options.response.aggregations.ranged.entries;
        ...
        defaults.uitemplate = function() {
            ...
            for ( var key in options.defaultquery.aggregations ) {
                var obj = options.defaultquery.aggregations[key];
            ...
            for ( var key in options.defaultquery.aggregations ) {
                if ( key != "range" && options.defaultquery.aggregations[key].term.node ) { // TODO: change this in case the facet is not a term type?
                    var node = options.defaultquery.aggregations[key].term;
        ...
        defaults.uibindings = function() {
            ...
            $('.query_string', obj).select2({
                "formatNoMatches": function() { return "type search terms and click enter to search, or choose a filter type to search";},
                "tags": function(q) {
                    ...
                    if ( field !== undefined ) {
                        ...
                        qry.aggregations.tags.facet_filter = {"query": options.query().query };
                        ...
                        if ( q.term.length ) {
                            if ( q.term.length == 1 ) {
                                ...
                                qry.aggregations.tags.facet_filter.query.bool.must.push(ts);
                                qry.aggregations.tags.terms.order = "term";
                            } else {
                        ...
                        if ( qry.aggregations.tags.facet_filter.query.bool.must.length == 0 ) {
                            delete qry.aggregations.tags.facet_filter;
                        };
                    };
                    ...
                    $.ajax({
                        ...
                        success: function(data) {
                            ...
                            var tags = [];
                            if ( field !== undefined ) {
                                var terms = data.aggregations.tags.terms;
